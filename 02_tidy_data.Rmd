...
In general programming parlance people refer to this as performing a _**join**_.  
...
#### Step 1: Load the blueprint file and select the relevant columns with correct answers  
...
#### Step 3: Reshape student answer into 'tidy' (long) format  
```{r}
...
```{r}
df.merged <- merge(x=results.long, y=df.blueprint, by="Question") %>% 
  dplyr::select(Cand.No, everything()) %>% 
  dplyr::arrange(Cand.No, Question)
print(df.merged)
```
