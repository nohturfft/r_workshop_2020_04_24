...
* (All three of the above packages are part of the [tidyverse](https://www.tidyverse.org/) family of packages developed by the folks at RStudio).  
...
To view the contents of the newly created data frame click on 'df.blueprint' in the RStudio Environment window. If you then look at the console, you will see that RStudio responded by running the `View(df.blueprint)` command. You can copy and paste that into your script if you would like to do this computationally in the future. Note that this output would not appear in the markdown output.  
